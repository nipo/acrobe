...
from acrobe.protocol.memory import UnsupportedAccess
...
def make_puppet():
    ...
    puppet = PicobootPuppet("picoboot", ram, Picoboot(transport))
...
class TestPicobootNode:
    """`Picoboot` Node wraps a transport and exposes the chip's
    address space — the puppet can be built on top of it."""

    @pytest.mark.asyncio
    async def test_transport_satisfies_protocol(self):
        assert isinstance(MockPicobootTransport(), PicobootTransport)

    @pytest.mark.asyncio
    async def test_bulk_family_reaches_transport(self):
        transport = MockPicobootTransport()
        node = Picoboot(transport)
        await node.mem_write(0x20000000, b"\x01\x02\x03\x04")
        data = await node.mem_read(0x20000000, 4)
        assert data == b"\x01\x02\x03\x04"
        assert transport.write_log == [(0x20000000, b"\x01\x02\x03\x04")]

    @pytest.mark.asyncio
    async def test_register_family_rejected(self):
        node = Picoboot(MockPicobootTransport())
        with pytest.raises(UnsupportedAccess):
            node.read32(0x20000000)

    @pytest.mark.asyncio
    async def test_delegates_exec(self):
        transport = MockPicobootTransport()
        node = Picoboot(transport)
        # The mock's exec walks the trampoline ABI, so a bare address
        # fails there — enough to show the call reached the transport.
