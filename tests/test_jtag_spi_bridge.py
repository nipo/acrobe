...
class TestSpiTransactor:
    ...
    async def test_cs_select(self):
        ...
        # First two bytes: DIVH + DIVL, third: CS select
        assert cmd[2] == 0x00  # SELECT slave 0, mode 0
        ...
        def respond(cmd):
            ...
            while i < len(cmd):
                ...
                if opcode & 0xC0 == 0xC0:  # SHIFT_INOUT
                    ...
                    i += 1 + count
                elif opcode & 0xE0 == 0x20:  # DIVH / DIVL
                    rsp.append(0x00)
                    i += 1
                else:
                    rsp.append(0x00)
    ...
    async def test_divisor_first_flush_only(self):
        ...
        assert first_cmd[0] & 0xE0 == 0x20  # DIVH
...
class TestSpiDivisorScheme:
    """The divisor register the emitted commands actually program, read
    back the way the RTL reads them."""

    @staticmethod
    def divisor_programmed(cmd, scheme):
        """Divisor register value after the RTL has consumed the
        leading divisor commands of `cmd`."""
        if scheme == "div5":
            assert cmd[0] & 0xe0 == 0x20
            return cmd[0] & 0x1f
        assert cmd[0] & 0xf0 == 0x20, "DIVH expected"
        assert cmd[1] & 0xf8 == 0x30, "DIVL expected"
        return ((cmd[0] & 0x0f) << 3) | (cmd[1] & 0x07)

    async def program(self, scheme, base_freq, ask):
        ch = MockChannel(lambda cmd: bytes(len(cmd)))
        codec = SpiTransactor(base_freq=base_freq, divisor_scheme=scheme)
        tr = _SpiFramedAdapter(codec, ch)
        reported = codec.freq_update(ask)
        await tr.post(Cs(0, mode=0))
        return codec, ch.sent_frames[0], reported

    @pytest.mark.parametrize("scheme,base_freq,ask", [
        ("divh_divl", 100e6, 25e6),
        ("divh_divl", 100e6, 1e6),
        ("divh_divl", 100e6, 100e3),   # below reach, clamps to slowest
        ("divh_divl", 100e6, 200e6),   # above reach, clamps to fastest
        ("div5", 30e6, 1e6),
        ("div5", 30e6, 100e3),
        ("div5", 30e6, 60e6),
    ])
    @pytest.mark.asyncio
    async def test_wire_divisor_matches_reported_rate(self, scheme, base_freq,
                                                      ask):
        codec, cmd, reported = await self.program(scheme, base_freq, ask)
        div = self.divisor_programmed(cmd, scheme)
        assert div == codec.divisor
        # One SCK period is two half-periods of (div+1) host cycles.
        assert reported == base_freq / ((div + 1) * 2)
        assert reported <= ask or div == codec.DIVISOR_MAX[scheme]

    @pytest.mark.asyncio
    async def test_divh_divl_is_two_commands(self):
        _codec, cmd, _reported = await self.program("divh_divl", 100e6, 25e6)
        assert cmd[:2] == bytes([0x20, 0x31])  # DIVH 0, DIVL 1 -> div 1
        assert cmd[2] == 0x00  # SELECT slave 0

    @pytest.mark.asyncio
    async def test_div5_is_one_command(self):
        _codec, cmd, _reported = await self.program("div5", 30e6, 1e6)
        assert cmd[:1] == bytes([0x2e])  # DIV 14
        assert cmd[1] == 0x00

    @pytest.mark.asyncio
    async def test_divh_divl_reaches_past_the_div5_range(self):
        codec, cmd, reported = await self.program("divh_divl", 100e6, 300e3)
        assert codec.divisor > 0x1f
        assert self.divisor_programmed(cmd, "divh_divl") == codec.divisor
        assert reported == 100e6 / ((codec.divisor + 1) * 2)

    def test_unknown_scheme_is_refused(self):
        with pytest.raises(ValueError):
            SpiTransactor(base_freq=30e6, divisor_scheme="divl")
...
class TestJtagFramedIntegration:
    ...
    async def test_spi_transaction_through_stack(self):
        ...
        # divisor(2 status) + cs_select(1 status) + shift_inout 1B (1 status + 1 miso) + cs_deselect(1 status)
        sim.load_rx([bytes([0x00, 0x00, 0x00, 0x00, 0x9F, 0x00])])
