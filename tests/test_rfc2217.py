...
class FakeSerial(SerialPort):
    ...
    def __init__(self, name="fake"):
        super().__init__(name)
        self._rx = bytearray()       # bytes injected (reads take from here)
        self._rx_event = asyncio.Event()
        self._tx = bytearray()       # bytes written (writes append here)
    ...
    async def flush_ops(self, batch):
        for op, future in batch:
            if isinstance(op, Write):
                ...
                if not future.done():
                    future.set_exception(TypeError(
                        f"FakeSerial: unsupported op {type(op).__name__}"))
