...
class Rtt(SerialPort):
    ...
    Data plane follows the Pipe contract: `read(size)` resolves with
    exactly `size` bytes, `read(None)` / `read_some(n)` with whatever
    the UP queue holds (at least one byte). `write(data)` pushes to
    the DOWN ring buffer, blocking while the buffer is full."""
    ...
    async def flush_ops(self, batch):
        # Both directions block indefinitely on target state (rx: no
        # UP bytes yet; tx: DOWN ring full, control block not found),
        # so every op runs in its own task — the per-direction locks
        # keep byte order.
            ...
            if isinstance(op, Write):
                ...
                if not future.done():
                    future.set_exception(TypeError(
                        f"Rtt: unsupported op {type(op).__name__}"))
    ...
    async def __rx_wait(self):
        """Block until at least one UP byte is queued. Raises
        RttError if the target advertises no UP buffer
        (MaxNumUpBuffers == 0)."""
    ...
    async def __deliver(self, op) -> bytes:
        if isinstance(op, ReadSome):
            await self.__rx_wait()
            n = min(op.max_size, len(self.__rx_queue))
        elif op.size is None:
            await self.__rx_wait()
            n = len(self.__rx_queue)
        else:
            await self.__rx_wait()
            while len(self.__rx_queue) < op.size:
                self.__rx_wakeup.clear()
                await self.__rx_wakeup.wait()
            n = op.size
