...
class SerialPort(Pipe, ABC):
    """Abstract async serial port.

    Superset of :class:`~acrobe.protocol.pipe.Pipe`: the data plane is
    Pipe's — `Read` / `Write` / `ReadSome` ops posted through the
    Batcher, concrete ports implement :meth:`flush_ops` — plus a
    control plane for line config, break, flush, modem signals, and
    registrable callbacks for signal edges and line-state errors.
    """
