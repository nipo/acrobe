...
class WriteOp:
    ...
    async def run(self, ep, mps):
        for offset in range(0, len(self.blob), mps):
            data = self.blob[offset:offset + mps]
            await ep.write(data)
...
class ReadOp:
    def __init__(self, size, *, min_size=None, timeout=1):
        ...
        self.min_size = (size or 1) if min_size is None else min_size
        self.blob = b""
        self.future = asyncio.Future()
...
class FtdiTransport:
    ...
    async def __writer_worker(self):
        while True:
            assert self.__read_queue.empty() or self.__reader
    ...
    def read(self, size: int | None, timeout: float|None = None):
    ...
    async def __reader_worker(self):
        while True:
            assert self.__write_queue.empty() or self.__writer
            async with self.__lock:
                try:
