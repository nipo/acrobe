...
# Byte-sized bulk transfers are capped much lower than the word-sized
# ones; the firmware silently truncates beyond this.
_MAX_RW8 = 64
...
class StLinkTransport:
    ...
    async def read_mem8(self, ap_num: int, addr: int,
                        length: int, csw: int = 0) -> bytes:
        """Read ``length`` bytes from ``addr`` via MEM-AP ``ap_num``
        using byte-sized bus accesses."""
        if length == 0:
            return b""
        if length > _MAX_RW8:
            raise ValueError(
                f"read_mem8 length {length} > {_MAX_RW8}")
        # A single-byte IN transfer wedges the endpoint on some
        # firmwares; ask for two and keep the first.
        transfer = max(length, 2)
        data = await self.command(
            self.__mem_command(protocol.DEBUG_READMEM_8BIT,
                               addr, transfer, ap_num, csw),
            transfer)
        await self.__get_last_rw_status()
        return data[:length]

    async def write_mem8(self, ap_num: int, addr: int, data: bytes,
                         csw: int = 0) -> None:
        """Write ``data`` to ``addr`` via MEM-AP ``ap_num`` using
        byte-sized bus accesses."""
        if not data:
            return
        if len(data) > _MAX_RW8:
            raise ValueError(
                f"write_mem8 length {len(data)} > {_MAX_RW8}")
        await self.__mem_write_raw(
            protocol.DEBUG_WRITEMEM_8BIT, ap_num, addr, data, csw)

    async def read_mem16(self, ap_num: int, addr: int,
                         length: int, csw: int = 0) -> bytes:
        """Read ``length`` bytes from ``addr`` via MEM-AP ``ap_num``
        using halfword-sized bus accesses. ST-Link v3 firmware only;
        older firmware answers with an error status."""
        if length == 0:
            return b""
        if addr % 2 or length % 2:
            raise ValueError(
                "read_mem16 needs a halfword-aligned address and length")
        data = await self.command(
            self.__mem_command(protocol.DEBUG_READMEM_16BIT,
                               addr, length, ap_num, csw),
            length)
        await self.__get_last_rw_status()
        return data

    async def write_mem16(self, ap_num: int, addr: int, data: bytes,
                          csw: int = 0) -> None:
        """Write ``data`` to ``addr`` via MEM-AP ``ap_num`` using
        halfword-sized bus accesses. ST-Link v3 firmware only."""
        if not data:
            return
        if addr % 2 or len(data) % 2:
            raise ValueError(
                "write_mem16 needs a halfword-aligned address and length")
        await self.__mem_write_raw(
            protocol.DEBUG_WRITEMEM_16BIT, ap_num, addr, data, csw)

    @staticmethod
    def __mem_command(opcode: int, addr: int, length: int,
                      ap_num: int, csw: int) -> bytes:
        return bytes([
            protocol.CMD_DEBUG,
            opcode,
            addr & 0xFF, (addr >> 8) & 0xFF,
    ...
    async def __mem_write_raw(self, opcode: int, ap_num: int, addr: int,
                              data: bytes, csw: int) -> None:
        cmd = self.__mem_command(opcode, addr, len(data), ap_num, csw)
        async with self.__lock:
            self.__logger.protocol(
                "ST-Link mem-write op=0x%02x ap=%d addr=0x%x len=%d",
                opcode, ap_num, addr, len(data))
            await self.__ep_out.write(cmd + bytes(_COMMAND_BYTES - len(cmd)))
            await self.__ep_out.write(data)
        await self.__get_last_rw_status()
