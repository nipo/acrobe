...
def _emit_cli(w, program):
    ...
    w.dedent()
    w.line()
    w.line("# Resolve to raw JTAG interface")
    w.line('if isinstance(leaf, JtagInterface):')
    w.indent()
    w.line('interface = leaf._interface')
    w.dedent()
    w.line('elif hasattr(leaf, "_interface"):')
    w.indent()
    w.line('interface = leaf._interface')
    w.dedent()
    w.line('else:')
    w.indent()
    w.line("click.echo(f'Root resolved to {type(leaf).__name__}, expected a JTAG interface.', err=True)")
