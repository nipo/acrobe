...
def _emit_helpers(w):
    ...
    w.indent()
    w.line('"""Shift IR register."""')
    w.line('self._did_operation = True')
    w.line('self._ir_value = value')
    w.line('self._ir_length = length')
    w.line('await self.tap.ir(value, dr_length=1)(read_tdo=False)')
    w.dedent()
    w.line()

    w.line('async def _dr_scan(self, tdi, length, capture=False):')
    w.indent()
    w.line('"""Shift DR with current IR value. Returns bytes if capture."""')
    w.line('self._did_operation = True')
    w.line('instr = self.tap.ir(self._ir_value, dr_length=length)')
    w.line('result = await instr(BitString(bytes(tdi), length), read_tdo=capture)')
    w.line('if result is not None:')
    w.indent()
    ...
    w.line()
    w.line('Only IDLE is implemented (maps to tap.run()). Other states')
    w.line('like DRPAUSE would require Interface-level access with')
    w.line('exclusive locking, which is not yet supported.')
    w.line('"""')
    w.line('for state in states:')
    w.indent()
    w.line("if state == 'IDLE':")
    w.indent()
    w.line('await self.tap.run()')
    w.dedent()
    w.line("elif state == 'RESET':")
    w.indent()
    w.line('await self._trst(None, None)')
    w.dedent()
    w.line('else:')
    w.indent()
    w.line("assert False, f'TAP state {state} not implemented'")
    w.dedent()
    w.dedent()
    w.dedent()
    w.line()
    ...
    w.line('if cycles:')
    w.indent()
    w.line('await self.tap.run(cycles)')
    w.dedent()
    w.line('if usecs:')
    w.indent()
    ...
    w.line('await self.tap.run(5)  # TODO: proper TMS reset sequence')
    w.dedent()
    w.line()
