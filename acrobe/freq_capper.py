...
class FreqCapper:
    ...
    ``option_set`` here consumes ``fmax=`` and ignores every other
    key; Node's option dispatch walks the MRO and calls each class's
    own ``option_set``, so no chaining is needed.
    """
    ...
    def freq_reapply(self):
        """Re-run freq_update() with the current effective constraint.

        For interfaces whose clock hardware only comes up in start():
        a cap recorded before then (an ``fmax=`` path option, say) was
        applied against nothing; calling this once the hardware exists
        pushes the constraint through unconditionally.
        """
        freq, _reason = self.__effective()
        self.__freq = self.freq_update(freq)
        return self.__freq
