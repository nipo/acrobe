...
class JtagTransactor:
    ...
    #   1000_0111  TDO_DELAY    host appends 1 byte (n-bit delay-1)
    CMD_SHIFT_BYTE   = 0x00
    ...
    CMD_DIVISOR      = 0x83
    CMD_SYS_RESET    = 0x84
    CMD_TDO_DELAY    = 0x87 # version 1 only
    CMD_RTI          = 0x90
    CMD_RESET        = 0x98
    CMD_RTI8         = 0xa0
    CMD_RESET8       = 0xb0
    ...
    def option_set(self, key, value):
        if key == "tdo_delay":
            tdo_delay = int(value)
            if self.__tdo_delay != tdo_delay:
                self.__tdo_delay_dirty = True
                self.__tdo_delay = tdo_delay
    ...
    def encode(self, batch) -> tuple[bytes, int, list]:
        ...
        if self.__tdo_delay_dirty and self.__version >= 1:
            cmd.append(self.CMD_TDO_DELAY)
            cmd.append(self.__tdo_delay & 0xff)
            rsp_size += 1
            self.__tdo_delay_dirty = False
...
from ....protocol import datagram, jtag
from ....util.pretty import sci_parse

@datagram.Datagram.db.register("nsl_jtag")
class JtagInterface(jtag.JtagInterface):
    def __init__(self, datagram, name: str = "jtag"):
    ...
    async def start(self):
        self.__transactor = JtagTransactor(self.__fin)
    ...
    async def flush_ops(self, batch):
        ...
        self.__decode_on_recv(self.__transport, self.__transactor, batch, cmd, gather)
