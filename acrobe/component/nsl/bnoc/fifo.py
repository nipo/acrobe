...
class JtagFifo:
    ...
    async def __exchange_speculative(self, tx_words, expect_frames):
        ...
        while frames_seen < expect_frames or tx_idx < len(tx_words):
            ...
            # TX shifts: never send more than the advertised free space. The
            # bridge contract is that these out_free words are accepted; we do
            # not trust it blindly but verify below.
            tx_shifts = []
            while (tx_idx + len(tx_shifts) < len(tx_words)
                   and len(tx_shifts) < out_free):
                word = tx_words[tx_idx + len(tx_shifts)]
                tx_shifts.append(
                    self.__data(self.VALID | self.READY | word, post_dr_run = 10))

            # Pad with dummy reads to drain the device's output queue.
            read_shifts = []
            while len(tx_shifts) + len(read_shifts) < min(in_ready, 128):
                read_shifts.append(self.__data(self.READY, post_dr_run = 10))

            if not tx_shifts and not read_shifts:
                # Nothing to do, run some cycles and retry
                await self.__tap.run(100)
                continue

            results = await asyncio.gather(*(tx_shifts + read_shifts))

            # Every VALID word we sent (<= out_free) must have been accepted
            # (READY set in its TDO). A dropped word cannot be resent -- it would
            # re-enter the FIFO after words already accepted and reorder the
            # frame -- so a miss is an unrecoverable flow-control violation.
            for i in range(len(tx_shifts)):
                if not (int(results[i]) & self.READY):
                    raise RuntimeError(
                        f"jtag fifo: tx word {i}/{len(tx_shifts)} rejected "
                        f"despite out_free={out_free}; bridge broke the "
                        f"flow-control contract")
            tx_idx += len(tx_shifts)
