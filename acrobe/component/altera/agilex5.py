    ...
    async def post_tlr(self):
        ...
        for tap in chain.children:
            ...
            if ctx.controller is not None:
                continue
            if not isinstance(tap, JtagDpTap):
                # Adjacent, unclaimed, but not an ARM JTAG-DP -- e.g.
                # another board TAP that simply happens to sit next to
                # us on the chain (a companion FPGA, not the HPS debug
                # port this hook exists to detect). Leave it alone.
                continue
            self.logger.note(
...
_AGILEX5_PARTS = {
    ...
    0x0363f0dd: "A5ED065BB23AE4SR0",  # Engineering Sample (ES), B23A package
}
