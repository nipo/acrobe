...
class MemoryMappedComponent(Node):
    ...
    * :data:`CoresightComponent.db` — keyed on DEVTYPE (class-0x9 legacy).
    ...
    # -- Register access ------------------------------------------
    #
    # ``reg_read`` / ``reg_write`` address the component's *own
    # aperture*: the argument is an offset from ``self.base``, not an
    # address in the bus's space. That is a different address space
    # from the one :class:`acrobe.protocol.memory.Interface` names
    # (``read32(addr)`` is absolute), so the aperture window keeps its
    # own verb pair rather than shadowing the bus surface — the same
    # split :meth:`acrobe.component.arm.ap.Ap.reg_read` draws.
    #
    # Both are thin delegates to ``self._bus``, which is the real
    # ``Interface`` implementer. A component does not batch on its own
    # behalf: a Batcher per CoreSight leaf would add a loop turn per
    # register access and coalesce nothing the bus below doesn't
    # already coalesce.

    def reg_read(self, offset: int):
        """Read the 32-bit register at ``self.base + offset``. Returns
        a Future that resolves with the register value. ``self._bus``
        is the path for absolute or out-of-aperture addresses."""
