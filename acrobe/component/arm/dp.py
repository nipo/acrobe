...
class DpSystemBus(memory.Interface):
    """Address space exposing the DP's own system addresses by issuing
    direct APACC ops on a :class:`Dp`. Used as the bus argument for
    ADIv6 top-level component discovery (BASEPTR0 walk) and for any
    non-MEM-AP component reachable via direct register access in the
    DP's system address space.

    Distinct from a MEM-AP: there is no CSW/TAR/DRW state machine —
    each access is one APACC at SELECT composed from ``addr[31:4]``.
    APACC has exactly one shape, a 32-bit aligned word transfer, so
    this surface takes only ``Read32`` / ``Write32`` from the register
    family and none of the bulk family: a byte-granular block has no
    meaning on the debug fabric, where every addressable thing is a
    component management register or a ROM Table entry.

    Accesses forward straight to the DP's batcher rather than through
    one of our own — an intermediate batching layer here would add a
    loop turn per ROM-walk register read and coalesce nothing."""

    ops = frozenset({memory.Read32, memory.Write32})
...
class Dp(Batcher, Node):
    ...
    def __init__(self, name: str = "dap", *, dpidr: int | None = None):
        ...
        self.__ap_probe_indices = list(range(16)) + list(range(240, 256))

    def option_set(self, key, value):
        if key == "ap_probe":
            self.__ap_probe_indices = [int(idx) for idx in value.split(";")]
    ...
    async def _enumerate_aps(self):
        ...
        futures = [
            ...
            for apsel in self.__ap_probe_indices
        ]
