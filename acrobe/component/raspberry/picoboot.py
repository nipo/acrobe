...
- `PicobootTransport` — the typing Protocol a USB-level PICOBOOT
  binding satisfies (`read`, `write`, `exec`). The actual USB
  binding lives elsewhere.
- `Picoboot` — the component Node wrapping a transport, exposing
  the chip's address space as an `acrobe.protocol.memory.Interface`.
...
class Picoboot(memory.Interface, BackgroundLowering, Batcher, Node):
    ...
    space through it. Only the bulk family: the bootrom's READ and
    WRITE commands are block transfers with a fixed 32-byte command
    frame each, so a "register" access would be a full USB round-trip
    for four bytes and is better spelled as the block it is.

    Sits under `PicobootAdapter` in the tree; discovered by the RP2040
    Target probe via `children_of_class(Picoboot)` and wrapped in a
        ...
        for op, future in batch:
            try:
                if isinstance(op, memory.ReadBlob):
                    result = await self.transport.read(op.addr, op.size)
                elif isinstance(op, memory.WriteBlob):
                    await self.transport.write(op.addr, op.data)
                    result = None
                else:
                    raise TypeError(
                        f"Picoboot can't lower {type(op).__name__}")
...
class PicobootPuppet(PuppetBase):
    ...
    workspace) and a `Picoboot` — memory access rides its batched
    address space, code launch rides its `exec`. The thunk is
    installed lazily on first `prepare()`.
    """
    ...
    def __init__(self, name, ram, picoboot: "Picoboot"):
    ...
    async def __install(self):
        ...
        await self.picoboot.mem_write(self.trampoline.address, blob)
        self.__installed = True

    async def mem_read(self, addr, size):
        return await self.picoboot.mem_read(addr, size)

    async def mem_write(self, addr, data):
        await self.picoboot.mem_write(addr, data)
    ...
    async def prepare(self, pc: int, *args):
        ...
        payload = struct.pack(
            "<6I", pc | 1, *padded, 0)
        await self.picoboot.mem_write(self.data.address, payload)
    ...
    async def run(self):
        ...
        self.__exec_task = asyncio.create_task(
            self.picoboot.exec(self.trampoline.address | 1))
    ...
    async def wait(self, timeout: float = 1.0):
        ...
        result_bytes = await self.picoboot.mem_read(
