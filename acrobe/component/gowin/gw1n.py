...
class GowinFpga(Tap, JtagSramFpga):
    ...
    async def status_read(self):
        with self.parent.parent.freq_capped("status", 1e6):
            status = self._status_type(int(await self.READ_STATUS(pre_dr_run = 6)))
    ...
    async def _sram_erase(self):
        ...
        await self.ISC_SRAM_ERASE(read_tdo=False, pre_dr_run = 100)
        await self.ISC_NOOP(read_tdo=False, pre_dr_run = 100)
        await asyncio.sleep(10e-3)
        await self.ISC_SRAM_ERASE_DONE(read_tdo=False, pre_dr_run = 100)
        await self.ISC_NOOP(read_tdo=False, pre_dr_run = 100)
...
async def _gowin_spi(tap):
    ...
    return JtagSpiBridge(ContinuousTransport(tap, tap.USER_IR[0]), base_freq=30e6)
